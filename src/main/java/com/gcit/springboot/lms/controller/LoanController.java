...
public class LoanController {
	...
	/*
	 Get one loan  
	@RequestMapping(value = "/loans/borrowers/{cardNo}/branches/{branchId}/books/{bookId}/date/{dateIn}", method = RequestMethod.GET)
	public Loan getLoan(@PathVariable int cardNo, @PathVariable int branchId, @PathVariable int bookId, @PathVariable String dateIn, HttpServletResponse response ) throws IOException {
		Timestamp di = new Timestamp(Long.parseLong(dateIn));
		logger.info("Getting loan with cardNo: " + cardNo + ", branchId: " + branchId + ", bookId: " + bookId + ", dateIn: " + di);
		Loan loan =  loanDao.getLoan(cardNo, branchId, bookId, di);
		if (loan == null) 
...
//	}
	...
	@RequestMapping(value = "/loans/borrowers/{cardNo}/branches/{branchId}/books/{bookId}", method = RequestMethod.PUT)
	public void updateLoan(@RequestBody @Valid Loan loan, @PathVariable int cardNo, @PathVariable int branchId, @PathVariable int bookId, HttpServletResponse response) throws IOException {
		logger.info("Updating loan with new due date: " + loan.getDueDate());
		try {
			//Timestamp di = new Timestamp(Long.parseLong(dateIn));
			int i = loanDao.updateLoan(cardNo, branchId, bookId, loan.getDueDate());
